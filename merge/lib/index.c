...
acc_index IdxCreate(int from, int to, int ofIt) {
   ...
   if (ofIt < from  || to < ofIt) {
      printf("from = %d, to = %d, ofIt = %d, No such index can be created.\n",
	    from, to, ofIt);
      exit(-1);
   }
...
acc_index IdxCreate_Copy(acc_index acc, int ofIt) {
   acc_index accIndex; 

   accIndex = (acc_index) HPAlloc(HPCreate(), sizeof(index));

   accIndex->from = acc->from; accIndex->to = acc->to;
   accIndex->ofIt = ofIt;
...
int IdxIntVal(acc_index accIndex) {
   if (accIndex == nil_Access) printf("nill index\n");
